...
export function createOrder({ table, items, note, guestName }) {
  ...
  const order = {
    id,
    table,
    guestName: (guestName || "").trim().slice(0, 40),
    ...
    printed: false,
    archived: false, // when true the kitchen UI hides it but history remains for /track
  };
...
export function getOrders() {
  // Return only non-archived orders for the kitchen UI
  return db.orders.filter(o => !o.archived);
}
...
export function buildEscPos(order) {
  ...
  if (order.guestName) { text(`Guest: ${order.guestName}`); push(LF); }
  text("--------------------------------"); push(LF);
